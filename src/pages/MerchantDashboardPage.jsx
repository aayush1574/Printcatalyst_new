...
export default function MerchantDashboardPage() {
  ...
  const [dashboardQrDataUrl, setDashboardQrDataUrl] = useState('');

  // Generate permanent QR code for the shop
  useEffect(() => {
    const slug = merchant?.slug || 'printsupport-hub';
    ...
    <div className="min-h-screen bg-[#0b0f19] text-slate-100 flex flex-col font-sans selection:bg-indigo-500 selection:text-white">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <main className="flex-1 flex overflow-hidden bg-slate-900/40 min-w-0">
          ...
          {/* ═══ TAB: SHOP QR STANDEE ═══ */}
          {activeTab === 'STANDEE' && (
            <div className="flex-1 p-4 sm:p-6 overflow-y-auto space-y-6 max-w-2xl mx-auto text-xs animate-fadeIn">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div>
                  ...
                  <p className="text-slate-400 text-xs">Permanent Shop QR for walk-in self-service printing</p>
                </div>

                <button
                  onClick={() => setIsStandeeOpen(true)}
                  className="px-4 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-bold flex items-center gap-2 shadow-lg shadow-indigo-600/30 self-start transition-all active:scale-95"
                >
                  <Sparkles className="w-4 h-4" />
                  <span>Open Standee Designer</span>
                </button>
              </div>

              {/* Dedicated Permanent QR Display Card */}
              <div className="glass-card p-6 sm:p-8 rounded-3xl border border-white/10 text-center space-y-6 shadow-2xl bg-slate-950/80">
                <div className="space-y-1">
                  <h3 className="text-base font-extrabold text-white tracking-wide">
                    {merchant?.name || 'Print Support'} Counter QR
                  </h3>
                  <p className="text-xs text-slate-400">
                    Customers scan this QR code to upload documents and send prints directly to your active queue.
                  </p>
                </div>

                {/* The Generated QR Code Display */}
                <div className="flex justify-center">
                  <div className="p-4 sm:p-5 bg-white rounded-3xl shadow-2xl border-4 border-indigo-500/40 inline-block hover:scale-[1.02] transition-transform">
                    {dashboardQrDataUrl ? (
                      <img
                      ...
                      <div className="w-56 h-56 sm:w-64 sm:h-64 flex items-center justify-center bg-slate-100 text-slate-400 rounded-xl">
                ...
                {/* Portal Link Pill */}
                <div className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-slate-900 border border-slate-800 text-indigo-300 font-mono text-xs max-w-full truncate">
                  <span className="truncate">{`${window.location.origin}/portal/${merchant?.slug || 'printsupport-hub'}`}</span>
                  <button
                    onClick={() => {
                      const url = `${window.location.origin}/portal/${merchant?.slug || 'printsupport-hub'}`;
                      navigator.clipboard.writeText(url);
                    ...
                    className="px-2.5 py-1 text-[11px] font-bold rounded-lg bg-indigo-600/30 hover:bg-indigo-600 text-indigo-300 hover:text-white transition-colors flex items-center gap-1 flex-shrink-0"
                  >
                    {copiedPortalLink ? (
                      <>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-2">
                  ...
                  <button
                    onClick={() => setIsStandeeOpen(true)}
                    className="py-3 px-4 rounded-xl bg-gradient-to-r from-indigo-600 to-cyan-500 hover:from-indigo-500 hover:to-cyan-400 text-white font-bold text-xs shadow-lg shadow-indigo-600/30 flex items-center justify-center gap-2 transition-all"
                  >
